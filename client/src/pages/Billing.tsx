...
import { CreditCard, Download, Plus, Loader2, Check, Crown, Calendar, AlertCircle, Wallet, Coins, Receipt, TrendingUp, Sparkles, ArrowUpRight, Clock, Globe } from "lucide-react";
...
export default function Billing() {
  ...
  return (
    <div className="space-y-8">
      ...
      {subscription && subscription.plan.name === "pro" && !subscription.cancelAtPeriodEnd && (subscription.stripeSubscriptionId || subscription.razorpaySubscriptionId || subscription.paypalSubscriptionId || subscription.paystackSubscriptionCode || subscription.mercadopagoSubscriptionId) && (
                  ...
                  {[...transactions].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).map((transaction, index) => (
                    <TableRow 
                      ...
                      className={`transition-colors hover:bg-slate-50 dark:hover:bg-slate-700/30 ${
                        index !== transactions.length - 1 ? 'border-b border-slate-100 dark:border-slate-700/50' : ''
                      }`}
                    >
