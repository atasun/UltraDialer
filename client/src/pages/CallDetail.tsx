...
import { ArrowLeft, ArrowRight, Play, Pause, Download, Loader2, Phone, Clock, Calendar, MessageSquare, PhoneIncoming, PhoneOutgoing, CheckCircle2, XCircle, User, Volume2, Heart, Target, Mail, Bot, ChevronLeft, ChevronRight } from "lucide-react";
...
interface Call {
  ...
  engine?: 'elevenlabs' | 'twilio-openai' | 'plivo-openai';
...
export default function CallDetail() {
  ...
  const hasRecording = call?.recordingUrl || call?.elevenLabsConversationId;
  ...
  useEffect(() => {
    ...
    if ((call?.recordingUrl || call?.elevenLabsConversationId) && id) {
  ...
  }, [call?.recordingUrl, call?.elevenLabsConversationId, id]);
  ...
  const contactName = contact && contact.firstName && contact.firstName.toLowerCase() !== 'unknown'
