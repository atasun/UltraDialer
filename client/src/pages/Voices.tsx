          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground mt-0.5">
                {isLoading 
                  ? t('voices.subtitleLoading')
                  : t('voices.subtitleWithCount', { count: accountVoices?.length || 0 })}
              </p>
            </div>
          </div>
        </div>
        ...
        {!isLoading && (
          <div className="flex items-center gap-2">
            <span className="text-sm text-muted-foreground">
              {debouncedSearch 
                ? t('voices.showingFiltered', { count: filteredVoices.length, total: accountVoices?.length || 0 })
                : t('voices.totalVoices', { count: accountVoices?.length || 0 })}
            </span>
          </div>
        )}
      </div>
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                {voice.preview_url && (
                  <Button
                    ...
                    className={playingVoice === voice.voice_id ? "bg-gradient-to-r from-green-500 to-emerald-500 text-white" : ""}
