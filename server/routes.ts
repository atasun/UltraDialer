...
import { setupDemoCallWebSocket } from "./services/demo-call/demo-audio-bridge";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Register demo call routes (browser-based demo for visitors)
  app.use(demoCallRoutes);
  ...
  // Setup Demo Call WebSocket for browser-based demo calls
  const demoCallWss = new WebSocketServer({ noServer: true });
  setupDemoCallWebSocket(demoCallWss);
  httpServer.on('upgrade', (request, socket, head) => {
    const pathname = new URL(request.url || '', `http://${request.headers.host}`).pathname;
    if (pathname.startsWith('/api/demo-call')) {
      demoCallWss.handleUpgrade(request, socket, head, (ws) => {
        demoCallWss.emit('connection', ws, request);
      });
    }
  });
  console.log('✅ Demo Call WebSocket initialized');
