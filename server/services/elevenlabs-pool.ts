 ...
 */
import { db } from "../db";
import { elevenLabsCredentials, agents, users } from "@shared/schema";
...
export class ElevenLabsPoolService {
  ...
  /**
   * Sync existing agents with null elevenLabsCredentialId to the pool
   */
  static async syncExistingAgents(): Promise<{ synced: number; total: number; credentialId: string | null }> {
    // Find all agents without an assigned credential
    const unassignedAgents = await db
      .select()
      .from(agents)
      .where(sql`${agents.elevenLabsCredentialId} IS NULL`);

    if (unassignedAgents.length === 0) {
      return { synced: 0, total: 0, credentialId: null };
    }
    ...
    // Assign all unassigned agents to the least-loaded credential
    await db
      .update(agents)
      .set({ elevenLabsCredentialId: credential.id })
      .where(sql`${agents.elevenLabsCredentialId} IS NULL`);

    // Update the totalAssignedAgents count
    await db
      .update(elevenLabsCredentials)
      .set({
        totalAssignedAgents: sql`${elevenLabsCredentials.totalAssignedAgents} + ${unassignedAgents.length}`,
        updatedAt: new Date(),
      })
      .where(eq(elevenLabsCredentials.id, credential.id));

    return {
      synced: unassignedAgents.length,
      total: unassignedAgents.length,
