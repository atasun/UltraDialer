...
export class DbStorage implements IStorage {
  ...
  async getGlobalAnalytics(timeRange: string): Promise<any> {
    ...
    switch (timeRange) {
      ...
      case '90d':
        ...
        break;
      case 'all':
        // For all time, use the earliest possible date
    ...
    // Fetch all data for overall totals
    ...
    // Fetch time-filtered data for calls and campaigns
    ...
    // Fetch previous period data for growth calculations (only if not all time)
    ...
    const calculateGrowthPercent = (current: number, previous: number): number => {
      if (previous === 0) {
        return current > 0 ? 100 : 0; // If previous was 0 and current > 0, that's 100% growth
      }
    ...
    // Calculate growthData - aggregate by date using ISO keys for proper year-spanning sorting
    const growthMap = new Map<string, { users: number; calls: number; campaigns: number }>();
    
    // Helper function to get ISO date key (YYYY-MM-DD) for proper sorting across year boundaries
    ...
    // Helper to get month key (YYYY-MM) for monthly aggregation
    const getMonthKey = (date: Date): string => {
      const d = new Date(date);
      return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
    };
    
    // Helper to format ISO date to display label
    ...
    // For all time, find the earliest data date and use monthly buckets
    let chartStartDate = startDate;
    let useMonthlyBuckets = false;
    
    if (isAllTime) {
      // Find the earliest date from any data
      const allDates: Date[] = [];
      ...
      if (allDates.length > 0) {
        ...
        // No data, default to last 12 months
        chartStartDate = new Date(now.getFullYear(), now.getMonth() - 11, 1);
      }
      useMonthlyBuckets = true;
    }
    
    // Get the ISO date string for startDate and now for boundary checks
    ...
    // Initialize bucket keys - simple approach: buckets are intervals from startDate
    // For monthly: every month; For weekly: every 7 days; For daily: every day
    ...
    // Ensure we have at least one bucket (for edge cases where startDate equals now)
    ...
    // Helper to find the correct bucket for a given date
    const getBucketKey = (date: Date): string | null => {
      if (useMonthlyBuckets) {
        // Monthly: return the month key
      ...
      if (groupByWeek) {
        // Find the bucket this date belongs to by reverse search
        // Each bucket covers [bucketKey, bucketKey + 6 days]
        for (let i = bucketKeys.length - 1; i >= 0; i--) {
          if (dateKey >= bucketKeys[i]) {
            return bucketKeys[i];
          }
        }
        return bucketKeys[0]; // Fallback to first bucket
      } else {
        // Daily: exact match - should always be in bucketKeys since we pre-initialized
    ...
    // Aggregate calls by creation date
    for (const call of filteredCalls) {
      if (call.createdAt) {
    ...
    // Aggregate campaigns by creation date
    for (const campaign of filteredCampaigns) {
    ...
    // Convert map to sorted array - ISO dates sort correctly as strings
    const growthData = Array.from(growthMap.entries())
      .sort((a, b) => a[0].localeCompare(b[0])) // ISO dates sort correctly as strings
      .map(([isoDate, data]) => ({ 
        date: formatDateLabel(isoDate, useMonthlyBuckets), // Convert to display format for output
        ...data 
      }));
    
    // Query userSubscriptions joined with plans for accurate pro/free counts
    // Only count subscriptions that are active AND have not expired
    ...
    // Build a set of user IDs with active non-free subscriptions
    const proUserIds = new Set<string>();
    for (const sub of activeSubscriptions) {
      // Count as pro if the plan name is not 'free'
  ...
  async getUserAnalytics(userId: string, timeRange: string = '7days', callType: string = 'all') {
    ...
    // Get all incoming connections for the user
    ...
    // Get ALL user calls using a comprehensive approach
    let allUserCalls: Call[] = [];
    
    // Get calls by direct userId ownership (most reliable)
    ...
    // Get campaign calls using parameterized inArray (prevents SQL injection)
    ...
    // Get incoming connection calls using parameterized inArray (prevents SQL injection)
    if (incomingConnectionIds.length > 0) {
      const incomingCalls = await db.select()
        .from(calls)
        .where(and(
          inArray(calls.incomingConnectionId, incomingConnectionIds),
          gte(calls.createdAt, startDate)
        ));
    ...
    // Also fetch Twilio+OpenAI calls and map to Call-compatible format
    const twilioOpenAICallsData = await db.select()
      .from(twilioOpenaiCalls)
      .where(and(
        eq(twilioOpenaiCalls.userId, userId),
        gte(twilioOpenaiCalls.createdAt, startDate)
      ));
    ...
    for (const toc of twilioOpenAICallsData) {
      ...
      allUserCalls.push(mappedCall);
    }

    // Also fetch Plivo+OpenAI calls and map to Call-compatible format
    const plivoAnalyticsCallsData = await db.select()
      .from(plivoCalls)
      .where(and(
        eq(plivoCalls.userId, userId),
        gte(plivoCalls.createdAt, startDate)
      ));
    ...
    // Helper to check if call is a batch call (via metadata)
    ...
    // Helper predicates for call type classification
    // Handle standard directions plus Twilio-specific statuses (bridged, simulcall, etc.)
    ...
    // Outgoing = explicit outgoing direction OR not incoming and not batch
    const isOutgoingCall = (c: Call): boolean => {
      // Batch calls are never counted as outgoing (they have their own category)
      if (isBatchCall(c)) return false;
      // Explicit outgoing direction (including Twilio 'outbound')
      if (outgoingDirections.includes(c.callDirection || '')) return true;
      // Campaign calls without incoming connection and not in incoming directions
      if (c.campaignId && !c.incomingConnectionId && !incomingDirections.includes(c.callDirection || '')) return true;
      // Any other user-owned call that isn't incoming is outgoing
    ...
    if (callType === 'incoming') {
      filteredCalls = allUserCalls.filter(isIncomingCall);
    } else if (callType === 'outgoing') {
      filteredCalls = allUserCalls.filter(isOutgoingCall);
    } else if (callType === 'batch') {
      filteredCalls = allUserCalls.filter(isBatchCall);
    }

    // Use filtered calls for metrics
    const allCalls = filteredCalls;
    
    // Calculate type breakdown for overview (using same predicates for consistency)
    const typeBreakdown = {
    ...
    // Count qualified leads (hot and warm)
    ...
    // Lead distribution - Hot, Warm, Cold, Lost (matching dashboard)
    const leadCounts = {
    ...
    // Campaign performance - show all campaigns, not just those with positive success rates
    ...
    // Daily calls for the period - count actual calls per day based on timeRange
    ...
    // For longer periods, aggregate by week/month to avoid too many data points
    if (daysToShow <= 14) {
      // Show daily for 7-14 days
      for (let i = daysToShow - 1; i >= 0; i--) {
        ...
        // Count calls that were created on this day (using FILTERED calls, not all)
  ...
  async getDashboardData(userId: string) {
    ...
    // Get ALL user calls (all-time for total counts)
    let allUserCalls: Call[] = [];
    
    // Get calls by direct userId ownership
    const directOwnershipCalls = await db.select()
      .from(calls)
    ...
    // Get incoming connection calls
    if (incomingConnectionIds.length > 0) {
      const incomingCalls = await db.select()
        .from(calls)
        .where(inArray(calls.incomingConnectionId, incomingConnectionIds));
      for (const call of incomingCalls) {
        if (!allUserCalls.find(c => c.id === call.id)) {
          allUserCalls.push(call);
        }
    ...
    // Also fetch Plivo+OpenAI calls and map to Call-compatible format
    const plivoCallsData = await db.select()
      .from(plivoCalls)
      .where(eq(plivoCalls.userId, userId));
    ...
    // For trends comparison, use last 2 weeks
    const prevWeekStart = new Date();
    prevWeekStart.setDate(now.getDate() - 14);
    
    // This week's calls (for trends)
    ...
    // Calculate avg duration and success rate for each type
    ...
    // Note: These this-week stats are used for trend comparison only
    const incomingStats = calcStats(incomingThisWeek);
    const outgoingStats = calcStats(outgoingThisWeek);
    const batchStats = calcStats(batchThisWeek);

    // Daily breakdown for the week chart (showing incoming vs outgoing)
    ...
    // Lead distribution (Hot, Warm, Cold, Lost) - ALL TIME
    const leadDistribution = {
    ...
    // Recent calls (last 10, with call direction info)
    ...
    // Get recent users (admin only - but we'll return empty for regular users)
    ...
    // Calculate campaign stats - total count, success rate, avg duration
    ...
    // Calculate campaign success rate based on completed campaigns with successful calls
    // Get all calls for all campaigns to compute overall success rate
    let allCampaignCalls: Call[] = [];
    if (campaignIds.length > 0) {
      allCampaignCalls = await db.select()
        .from(calls)
        .where(inArray(calls.campaignId, campaignIds));
    }
    ...
    // Get forms and form submissions count
    ...
    // Get templates count (user's own templates + system templates)
